...
from tidal_dl_sg.helper.path import check_file_exists
...
class TestCheckFileExistsCaseSensitive:
    ...
    def test_exact_match_found(self, audio_path):
        with patch("tidal_dl_sg.helper.path.os.path.isfile", return_value=True):
            assert check_file_exists(audio_path) is True

    def test_exact_match_not_found(self, audio_path):
        with patch("tidal_dl_sg.helper.path.os.path.isfile", return_value=False):
    ...
    def test_extension_ignore_found(self, audio_path):
        ...
        with patch("tidal_dl_sg.helper.path.os.path.isfile", side_effect=side_effects):
    ...
    def test_extension_ignore_calls_all_extensions(self, audio_path):
        ...
        with patch("tidal_dl_sg.helper.path.os.path.isfile", return_value=False) as mock_isfile:
...
class TestCheckFileExistsCaseInsensitive:
    ...
    def test_exact_match_found(self, parent_dir):
        ...
        with (
            patch("tidal_dl_sg.helper.path.os.listdir", return_value=["Song.flac"]),
            patch("tidal_dl_sg.helper.path.os.path.isfile", return_value=True),
        ):
    ...
    def test_all_uppercase_vs_lowercase(self, parent_dir):
        ...
        with (
            patch("tidal_dl_sg.helper.path.os.listdir", return_value=["MUSIC.FLAC"]),
            patch("tidal_dl_sg.helper.path.os.path.isfile", return_value=True),
        ):
            assert check_file_exists(p, case_sensitive=False) is True
    ...
    def test_empty_directory(self, parent_dir):
        ...
        with (
            patch("tidal_dl_sg.helper.path.os.listdir", return_value=[]),
            patch("tidal_dl_sg.helper.path.os.path.isfile", return_value=True),
        ):
            assert check_file_exists(p, case_sensitive=False) is False
